...
def learning_snapshot(domain="All"):
    ...
    for d in domains("Both"):
        ...
        pct=sum(int(r["correct"]) for r in drows)/len(drows); low=sum(1 for r in drows if int(r["correct"])==1 and int(r["confidence"] or 0)==1)
        if pct<.70:weak.append((pct,d))
        if low>=2:fragile.append((low,d))
    recent_misses=[dict(r) for r in rows if int(r["correct"])==0][:5]; c.close(); weak.sort(); fragile.sort(reverse=True); return {"weak_domains":[d for _,d in weak[:3]],"fragile_domains":[d for _,d in fragile[:3]],"recent_misses":recent_misses}

def grade(question_id,selected,confidence=0):
    q=get_question(question_id); correct=int(selected.upper()==q["answer"]); c=db(); c.execute("INSERT INTO attempts(ts,question_id,part,domain,difficulty,selected,correct,confidence) VALUES(?,?,?,?,?,?,?,?)",(datetime.now().isoformat(timespec="seconds"),question_id,q["part"],q["domain"],q["difficulty"],selected.upper(),correct,int(confidence or 0))); c.commit(); c.close(); return correct,q
