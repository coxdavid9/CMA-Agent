...
def _question_history(conn):
 rows=conn.execute("SELECT question_id,COUNT(*) attempts,SUM(correct) correct FROM attempts GROUP BY question_id").fetchall(); return {r[0]:{"attempts":int(r[1]),"correct":int(r[2] or 0)} for r in rows}


def question_status(question_id,conn):
 h=_question_history(conn).get(question_id,{"attempts":0,"correct":0}); attempts=h["attempts"]; results=[int(r[0]) for r in conn.execute("SELECT correct FROM attempts WHERE question_id=? ORDER BY id DESC LIMIT 5",(question_id,)).fetchall()]
