...
def _postgres_connection():
    ...
    return psycopg.connect(DATABASE_URL, row_factory=dict_row, connect_timeout=10, sslmode="require")


def _postgres_execute(conn, sql, params=()):
    # Keep the existing SQLite-style ? placeholders throughout the engine.
    # psycopg uses %s, so translate placeholders only at the DB boundary.
    return conn.execute(sql.replace("?", "%s"), params)


class _PostgresConnection:
    def __init__(self):
        self._conn=_postgres_connection()

    def execute(self, sql, params=()):
        return _postgres_execute(self._conn, sql, params)
...
def get_or_start_session():
    ...
    if not r or not r["last_activity"] or now-datetime.fromisoformat(r["last_activity"])>timedelta(minutes=SESSION_TIMEOUT_MINUTES):
        sid=str(uuid.uuid4()); c.execute("INSERT INTO session_state(id,session_id,last_activity) VALUES(1,?,?) ON CONFLICT(id) DO UPDATE SET session_id=EXCLUDED.session_id,last_activity=EXCLUDED.last_activity",(sid,now.isoformat(timespec="seconds"))) if DATABASE_URL else c.execute("INSERT OR REPLACE INTO session_state(id,session_id,last_activity) VALUES(1,?,?)",(sid,now.isoformat(timespec="seconds")))
    else:
...
def start_new_session():
    c=db(); sid=str(uuid.uuid4()); now=datetime.now(); c.execute("INSERT INTO session_state(id,session_id,last_activity) VALUES(1,?,?) ON CONFLICT(id) DO UPDATE SET session_id=EXCLUDED.session_id,last_activity=EXCLUDED.last_activity",(sid,now.isoformat(timespec="seconds"))) if DATABASE_URL else c.execute("INSERT OR REPLACE INTO session_state(id,session_id,last_activity) VALUES(1,?,?)",(sid,now.isoformat(timespec="seconds"))); c.commit(); c.close(); return sid
...
def save_preferences(part,difficulty,domain):
    c=db()
    if DATABASE_URL:
        c.execute("INSERT INTO preferences(id,part,difficulty,domain,resume_question_id) VALUES(1,?,?,?,(SELECT resume_question_id FROM preferences WHERE id=1)) ON CONFLICT(id) DO UPDATE SET part=EXCLUDED.part,difficulty=EXCLUDED.difficulty,domain=EXCLUDED.domain",(part,difficulty,domain))
    else:
        c.execute("INSERT OR REPLACE INTO preferences(id,part,difficulty,domain,resume_question_id) VALUES(1,?,?,?,COALESCE((SELECT resume_question_id FROM preferences WHERE id=1),NULL))",(part,difficulty,domain))
    c.commit(); c.close()
...
def save_goal(target_date,part,daily_minutes):
    ...
    if DATABASE_URL:
        c.execute("INSERT INTO goals(id,target_date,part,daily_minutes) VALUES(1,?,?,?) ON CONFLICT(id) DO UPDATE SET target_date=EXCLUDED.target_date,part=EXCLUDED.part,daily_minutes=EXCLUDED.daily_minutes",(target_date,part,daily_minutes))
    else:
