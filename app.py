...
def clean_question(text):
    text=re.sub(r'\s*:\s*(?=(?:a|an|the)\b)',' ',text,flags=re.IGNORECASE)
...
with dashboard:
    ...
    if snap['weak_domains']:
        for t in snap['weak_domains']:
            st.markdown(f'<div class="card"><strong>🔴 {t}</strong><br><span class="muted">Below your current learning threshold — prioritize this area.</span></div>',unsafe_allow_html=True)
    elif snap['fragile_domains']:
        for t in snap['fragile_domains']:
            st.markdown(f'<div class="card"><strong>🟡 {t}</strong><br><span class="muted">You are getting these right, but confidence is still fragile.</span></div>',unsafe_allow_html=True)
    else:
    ...
    bars={'New':0,'Learning':.6,'Weak':.35,'Mastered':1}
    ...
    with st.expander('Recent mistakes'):
        if snap['recent_misses']:
            for m in snap['recent_misses']:
                st.write(f"• **{m['domain']}** · confidence {m['confidence']}/3")
        else:
...
with study:
    ...
    if 'message' not in p:
        ...
        for t in p['focus_topics']:
            st.markdown(f'<div class="card">🎯 <strong>{t}</strong></div>',unsafe_allow_html=True)
    ...
    if history:
        for h in history:
            st.markdown(f'<div class="card"><strong>📚 {h["target_date"]} · {h["part"]} · {h["daily_minutes"]} min/day</strong><br><span class="muted">Saved {h["created_at"]}</span></div>',unsafe_allow_html=True)
    else:
