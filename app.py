...
from cma_agent.engine import (
    ...
    learning_snapshot, stats, plan, save_goal, recent,
...
with st.sidebar:
    ...
    # Persist the last-used settings so they survive closing/reopening the app.
...
with practice:
    ...
    if q:
        ...
        answer = st.radio("Select your answer", list(q["choices"].keys()), format_func=lambda x: f"{x}. {q['choices'][x]}", key="answer_widget")
        confidence = st.radio("How confident are you?", [1, 2, 3, 4, 5], format_func=lambda x: {1: "1 — Guessing", 2: "2 — Not sure", 3: "3 — Somewhat confident", 4: "4 — Confident", 5: "5 — Very confident"}[x], horizontal=True, key="confidence_widget")
        st.session_state.confidence_value = confidence

        if not st.session_state.submitted and st.button("Submit & Learn", type="primary", use_container_width=True):
        ...
        if st.session_state.submitted:
            ...
            feedback = learning_feedback(q["id"], st.session_state.selected, st.session_state.confidence_value)
            ...
            if not ok:
                ...
                st.write("Don't immediately move on. Try a different question from the same domain to see whether you can apply the concept.")
            else:
                st.write("Because you reported your confidence, the coach can distinguish solid knowledge from an answer you happened to get right.")
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🎯 Give Me a Targeted Follow-up", type="primary", use_container_width=True):
                    st.session_state.followup = choose_followup(q["id"], st.session_state.selected, part, difficulty)
...
with dashboard:
    ...
    if snap["weak_domains"]: st.error("🔴 Priority: " + " · ".join(snap["weak_domains"]))
    else: st.success("No clear weak domain yet — keep building your sample.")
    if snap["fragile_domains"]: st.warning("🟡 Fragile knowledge: " + " · ".join(snap["fragile_domains"]))
    ...
    if snap["recent_misses"]:
        for miss in snap["recent_misses"][:5]: st.write(f"• **{miss['domain']}** — missed {miss['question_id']} (confidence {miss['confidence']}/5)")
    else: st.info("Complete a few questions and the coach will start identifying patterns.")
...
with study:
    ...
    with st.form("goal_form"):
        ...
        if st.form_submit_button("Save Goal", type="primary"):
            save_goal(target.isoformat(), goal_part, int(minutes)); st.success("Study goal saved.")
    p = plan()
    if "message" not in p:
        x, y = st.columns(2); x.metric("Days Remaining", p["days_remaining"]); y.metric("Daily Minutes", p["daily_minutes"])
        st.markdown("### Focus Areas")
        for topic in p["focus_topics"]: st.write(f"🎯 {topic}")
        st.markdown("### Suggested Session")
        for item in p["session"]: st.write(f"• {item}")
    else: st.info("Save your exam target and daily study time to generate a plan.")
