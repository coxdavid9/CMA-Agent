...
if 'session_id' not in st.session_state: st.session_state.session_id=get_or_start_session()
...
with st.sidebar:
    ...
    if st.session_state.question: st.divider(); st.caption('Current session'); st.write(f"**{st.session_state.question['domain']}**"); st.caption('Session resets after 30 minutes of inactivity.')
...
with practice:
    ...
    if q is None:
        ...
        if not st.session_state.submitted:
            if st.button('Submit Answer',type='primary',use_container_width=True): ok,graded=grade(q['id'],answer,confidence,st.session_state.session_id); st.session_state.submitted=True; st.session_state.selected=answer; st.session_state.confidence_value=confidence; st.session_state.result=(ok,graded); st.rerun()
        else:
...
with dashboard:
    s=stats(); snap=learning_snapshot(); current=session_stats(st.session_state.session_id)
    st.markdown('<div class="section">Current Session</div>',unsafe_allow_html=True)
    a,b=st.columns(2); a.markdown(f'<div class="stat"><div class="label">Questions This Session</div><div class="value">{current["attempted"]}</div></div>',unsafe_allow_html=True); b.markdown(f'<div class="stat"><div class="label">Session Accuracy</div><div class="value">{current["accuracy"]}%</div></div>',unsafe_allow_html=True)
    if current['attempted']: st.caption('Session resets after 30 minutes of inactivity.')
    else: st.info('No questions answered in the current session yet.')
    st.markdown('<div class="section">Historical Progress</div>',unsafe_allow_html=True)
    a,b,c=st.columns(3); a.markdown(f'<div class="stat"><div class="label">Lifetime Accuracy</div><div class="value">{s["accuracy"]}%</div></div>',unsafe_allow_html=True); b.markdown(f'<div class="stat"><div class="label">Questions Completed</div><div class="value">{s["attempted"]}</div></div>',unsafe_allow_html=True); c.markdown(f'<div class="stat"><div class="label">Question Bank</div><div class="value">{s["question_bank_size"]}</div></div>',unsafe_allow_html=True)
...
with study:
    ...
    history=goal_history(); st.markdown('<div class="section">Historical Study Plans</div>',unsafe_allow_html=True)
    if history:
